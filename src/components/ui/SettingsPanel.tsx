    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
          <h2 id="settings-panel-title" className="text-base font-semibold text-foreground">
            {title}
          </h2>
          <button
            onClick={() => onOpenChange(false)}
...
export const SettingsCategory: React.FC<SettingsCategoryProps> = ({
  ...
  return (
    <div className="border-b border-border last:border-b-0">
      ...
      >
        {icon && <span className="w-5 h-5 text-muted-foreground shrink-0">{icon}</span>}
...
export const SettingsInfo: React.FC<SettingsInfoProps> = ({ label, value, action }) => {
  return (
    <div className="px-3 py-2">
      <div className="text-xs text-muted-foreground mb-1">{label}</div>
      <div className="flex items-center gap-2">
        <code className="flex-1 text-sm text-foreground font-mono truncate">{value}</code>
        {action && (
          <button
