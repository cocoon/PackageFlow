...
const strategyConfig: Record<ToolchainStrategy, { label: string; color: string; bgColor: string }> =
  {
    volta_priority: {
  ...
  const handleEnableCorepack = async () => {
    ...
    try {
      ...
      if (result.success) {
        setOperationMessage({
          type: 'success',
          text: result.message || 'Corepack enabled successfully',
        });
        // Reload health check
  ...
  const handleFixPnpmHomeConflict = async () => {
    ...
    try {
      ...
      if (result.success) {
        setOperationMessage({
          type: 'success',
          text: result.message || 'PNPM HOME conflict fixed',
        });
        // Reload health check
        await loadHealthCheck();
      } else {
    ...
    try {
      ...
      try {
        const settings = await invoke<{
          toolchain_preferences?: Record<string, ProjectPreference>;
        }>('get_settings_value', { key: 'toolchain_preferences' });
  ...
  const handleConfirmDelete = async () => {
    ...
    if (deleteTarget.type === 'single' && deleteTarget.projectPath) {
      ...
      try {
        ...
        setPreferences((prev) => prev.filter((p) => p.project_path !== deleteTarget.projectPath));
      } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto space-y-6 min-h-0">
        ...
        {!isLoading && preferences.length === 0 && (
          <div className="p-8 rounded-lg border border-border bg-card text-center">
            ...
            <h3 className="text-sm font-medium text-foreground mb-1">No Saved Preferences</h3>
            <p className="text-xs text-muted-foreground max-w-xs mx-auto">
              When you choose "Remember this choice" in the Version Mismatch dialog, your preference
              will be saved here.
            </p>
          </div>
        )}
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between py-2 border-b border-border">
                ...
                <div className="flex flex-col items-end gap-0.5">
                  ...
                  >
                    {diagnostics?.volta.available
                      ? diagnostics.volta.version
                        ? `v${diagnostics.volta.version}`
                        : 'Installed'
                      : 'Not installed'}
                  </span>
              ...
              <div className="flex items-center justify-between py-2 border-b border-border">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {diagnostics?.corepack.available && !diagnostics?.corepack.enabled && (
                    ...
                    >
                      {isEnablingCorepack ? <Loader2 className="w-3 h-3 animate-spin" /> : 'Enable'}
                    </Button>
                  )}
                </div>
              </div>
        ...
        <div className="p-4 bg-muted/30 rounded-lg border border-border">
          <h3 className="text-sm font-medium text-foreground mb-2">About Toolchain Preferences</h3>
          <ul className="text-sm text-muted-foreground space-y-1">
            <li>
              - <strong>Volta Priority</strong>: Use Volta to manage Node.js and package manager
              versions
            </li>
            <li>
            ...
            <li>
              - <strong>Hybrid Mode</strong>: Volta manages Node.js, Corepack manages package
              managers
            </li>
            <li>
      ...
      <ConfirmDialog
        ...
        variant="destructive"
        title={deleteTarget?.type === 'all' ? 'Clear All Preferences' : 'Remove Preference'}
        description={
