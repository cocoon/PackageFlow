...
export function useProject(): UseProjectReturn {
  ...
  const [workspaces, setWorkspaces] = useState<Map<string, WorkspacePackage[]>>(
  ...
  const saveLastProjectId = useCallback(
  ...
  const addProject = useCallback(
    async (path: string): Promise<ScanProjectResponse> => {
      setIsLoading(true);
      setErrorWithAutoDismiss(null);
  ...
  const updateLastOpenedAt = useCallback(
    async (id: string) => {
  ...
  const updateProject = useCallback(
    async (id: string, updater: (project: Project) => Project): Promise<Project | null> => {
      ...
      try {
        ...
        if (response.success && response.project) {
            ...
            if (response.workspaces && response.workspaces.length > 0) {
              workspacesWithFramework = await Promise.all(
                response.workspaces.map(async (workspace) => {
                  const { uiFramework: wsUiFramework } = await detectProjectFrameworks(
          ...
          setErrorWithAutoDismiss(
            errorMessages[response.error || ''] || 'Failed to refresh project'
          );
          return false;
        }
      } catch (err) {
  ...
  const getProjectWorkspaces = useCallback(
    (projectId: string): WorkspacePackage[] => {
