...
PackageFlow 的「时间机器（Time Machine）」会在 lockfile 变更时自动捕获依赖快照，让你追踪依赖演进、检测潜在风险，并在不同状态之间对比差异。
...
当项目的 lockfile 发生变更时，PackageFlow 会自动：
...
控制是否对所有项目启用 lockfile 自动监控。启用后，PackageFlow 会监视 lockfile 并在变更时捕获快照。
