...
//
// This server uses the shared_store module for atomic writes and data integrity.
...
use packageflow_lib::utils::shared_store::{
    // Store operations
    get_app_data_dir, read_store_data as read_shared_store,
    write_store_data_validated, SharedStoreData,
    // Error handling
    sanitize_error,
    // Input validation
...
fn get_mcp_log_path() -> Result<PathBuf, String> {
    ...
    Ok(app_dir.join("mcp-requests.log"))
}
...
/// Log an MCP request to file (enhanced audit logging)
///
/// Records:
...
///
/// Also performs automatic log rotation when file exceeds 10MB
fn log_request(
    tool_name: &str,
    ...
    let log_path = match get_mcp_log_path() {
        Ok(path) => path,
        Err(_) => return, // Silently fail if we can't get log path
    };
    ...
    // Perform log rotation if needed (logs > 10MB)
...
fn read_store_data() -> Result<StoreData, String> {
    ...
    Ok(StoreData {
        ...
        projects: shared_data
            ...
            .collect(),
        workflows: shared_data
            .workflows
            .into_iter()
            .filter_map(|v| serde_json::from_value(v).ok())
            .collect(),
        running_executions: serde_json::from_value(shared_data.running_executions).unwrap_or_default(),
        settings: shared_data.settings,
        security_scans: serde_json::from_value(shared_data.security_scans).unwrap_or_default(),
        custom_step_templates: shared_data.custom_step_templates.into_iter().map(|t| {
            CustomStepTemplate {
                id: t.id,
                name: t.name,
                command: t.command,
        ...
        mcp_config: shared_data.mcp_config.unwrap_or_default(),
    })
}

/// Write store data to disk atomically with validation
///
/// Uses the shared store module which provides:
/// - Data integrity validation before write
...
fn write_store_data(data: &StoreData) -> Result<(), String> {
    ...
    let shared_data = SharedStoreData {
        ...
        workflows: data
            ...
            .collect(),
        running_executions: serde_json::to_value(&data.running_executions).unwrap_or_default(),
        settings: data.settings.clone(),
        security_scans: serde_json::to_value(&data.security_scans).unwrap_or_default(),
        mcp_config: Some(data.mcp_config.clone()),
        custom_step_templates: data.custom_step_templates.iter().map(|t| {
            packageflow_lib::models::step_template::CustomStepTemplate {
                id: t.id.clone(),
                name: t.name.clone(),
